%EEG remove bad channels and bad portions of data -STEP 6
...
for i = 1 : length(subject)
    for j = 1 : length(task)
        ...
        EEG = pop_loadset('filename', num2str(filename),'filepath','D:\\DATA_ARRANGED\\EEG_trimmed\\');
        [ALLEEG, EEG, CURRENTSET] = eeg_store( ALLEEG, EEG, 0 );
        EEG.originalEEG = EEG;
        EEG = pop_clean_rawdata(EEG, 'FlatlineCriterion',5,'ChannelCriterion',0.8,'LineNoiseCriterion',4, ...
            'Highpass','off','BurstCriterion',20,'WindowCriterion',0.25,'BurstRejection','off','Distance','Euclidian', ...
            'WindowCriterionTolerances',[-Inf 7] );
        [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 1,'setname',num2str(setname),'overwrite','on','gui','off');
        EEG.clean = "yes";
        EEG = pop_saveset( EEG, 'filename', num2str(savename),'filepath','D:\\DATA_ARRANGED\\EEG_clean\\');
