...
for i = 1 : length(subject)
    for j = 1 : length(task)
		...
		filename = strcat( subject{i}, lab, task{j}, RESPext, trimext, filtext, detext, setext);
		setname = strcat( subject{i}, lab, task{j}, RESPext, trimext, filtext, detext);

		EEG = pop_loadset('filename', num2str(filename),'filepath','C:\\DATA_ARRANGED\\RESP_detected\\');
		[ALLEEG, EEG, CURRENTSET] = eeg_store( ALLEEG, EEG, 0 );

        beats_per_second = length(EEG.HEP.qrs) / EEG.xmax;
        meanRR = beats_per_second * 60;
...
%cleaning sub07 and sub14 grazME as they are too noisy 
% meanRR_grazME(7) = NaN;
