...
for i = 1 : length(subject)
    for j = 1 : length(task)
		...
		filename = strcat( subject{i}, lab, task{j}, PPGext, trimext, filtext, detext, setext);
		setname = strcat( subject{i}, lab, task{j}, PPGext, trimext, filtext, detext);

		EEG = pop_loadset('filename', num2str(filename),'filepath','D:\\DATA_ARRANGED\\PPG_detected\\');
...
%cleaning sub07 and sub14 grazME as they are too noisy AND SUB9
meanRR_grazME(7) = NaN;
meanRR_grazME(12) = NaN;
...
% RESULTS.PPG.IBI.horizontally = A;
% RESULTS.PPG.IBI.vertically = A';
% RESULTS.PPG.IBI.mean = meanRR_values;
% RESULTS.PPG.HR.horizontally = B;
% RESULTS.PPG.HR.vertically = B';
% 
